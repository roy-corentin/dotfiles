...
#+begin_src conf
  ...
  windowrule = float on, match:class ^Wiremix$
  windowrule = float on, match:class ^foot-float$
  ...
  windowrule = center on, match:class ^(org.freedesktop.impl.portal.desktop.kde)$
  windowrule = size 1280 720, match:class ^(org.freedesktop.impl.portal.desktop.kde)$
  windowrule = size 700 500, match:class ^(org.pulseaudio.pavucontrol|Wiremix)$

  windowrule = float on, match:class ^org.kde.gwenview$
  windowrule = size 65% 65%, match:class ^org.kde.gwenview$
  ...
  windowrule = tag +floating-window, match:class (blueberry.py|org.gnome.NautilusPreviewer|com.gabm.satty|About|TUI.float)
  windowrule = tag +floating-window, match:class (xdg-desktop-portal-gtk|sublime_text|DesktopEditors|org.gnome.Nautilus|waypaper|Wiremix), match:title ^(Open.*Files?|Open [F|f]older.*|Save.*Files?|Save.*As|Save|All Files|.*wants to [open|save].*|[C|c]hoose.*)

  # Popped window rounding
  windowrule = rounding 8, match:tag pop

  # No Screen share windows
  ...
  windowrule = float on, match:class ^(brave)$, match:title ^(Picture in picture)$
  windowrule = float on, match:class ^(zen)$, match:title ^(Picture-in-Picture)$
  windowrule = size 740 440, match:class ^(zen)$, match:title ^(Picture-in-Picture)$
  windowrule = move onscreen 100%-w-20, match:class ^(zen)$, match:title ^(Picture-in-Picture)$
  windowrule = float on, match:class ^brave-.*-Default$

  # Emacs
  windowrule = float on, match:class ^(emacs)$, match:title ^(Ediff.*)$
  windowrule = size 780 410, match:class ^(emacs)$, match:title ^(Ediff.*)$
  windowrule = move onscreen 100%-w-20, match:class ^(emacs)$, match:title ^(Ediff.*)$
  windowrule = float on, match:class ^(emacs)$, match:title ^(emacs-float)$

  # Message App
  windowrule = workspace 1, match:class ^(.*Microsoft Teams.*|teams-for-linux|discord|WebCord|Slack)$
  windowrule = workspace 1, match:class ^()$
  ...
  windowrule = move 100%-w-70 100%-h-45, match:title WebcamOverlay # There's a typo in the hyprland rule so 100%-w on the height param is actually correct here
...
#+begin_src conf
  ...
  $appmenu = ~/.config/hypr/scripts/walker-menu.sh
  ...
  $editor-term = $term2 -e emacsclient -t -a 'emacs -nw'
...
#+begin_src conf
  ...
  bindd = $mainMod SHIFT, D, Launch AppMenu, exec, hyprlauncher
